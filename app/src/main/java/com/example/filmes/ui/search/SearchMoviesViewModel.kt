...
@HiltViewModel
class SearchMoviesViewModel @Inject constructor(private val repository: MoviesRepository)
    :ViewModel() {
            ...
            try {
                when (val response = repository.searchMovies(query)) {
                    ...
                    is Resource.Error -> {
                        ...
                        Log.e(
                            "Network",
                            "searchMovies: Failed getting filmes Carregando?= $carregando"
                        )
                    }
                ...
                Log.d(
                    "Network",
                    "searchMovies: ${exception.message.toString()} Carregando?= $carregando"
                )
            }
